...
with col2:
    st.image("utils/mistral_logo.png", use_container_width=True)
    st.markdown(f"[Visit Mistral AI]({mistral_url})", unsafe_allow_html=True)

input_papers = st.file_uploader(
    label="Add Your Research Papers", type="pdf", accept_multiple_files=True
)
    ...
    for status, summary, metadata, kg_overview in messages:
        if "Failed" in status:
            ...
            clean_metadata = (
                ...
                .strip('"""')
                .strip()
                .replace('"Here is the metadata of the Paper":\n', "")
            )
