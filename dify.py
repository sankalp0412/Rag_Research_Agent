...
def upload_papers(files: List[IO], dummy_response:bool) -> List[Tuple[str, str | None, str | None, str | None]]:
...
def use_dify(file: IO) -> Tuple[str, str | None, str | None, str | None]:
    ...
    if file_id:
        ...
        return (
            f"Upload failed for {os.path.basename(file.name)}",
            None,
            None,
            None
        )
...
def run_dify_workflow(file_id: str) -> Tuple[str, str | None, str | None, str | None]:
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            if status == "failed":
                print(f"Upload to knowledge graph failed due to: {res_json['data']['error']}")
                return "failed", None, None, None
            return handle_output(res_json)
        else:
            print(f"Error calling workflow API: {response.status_code} - {response.text}")
            return "failed", None, None, None
...
def handle_output(workflow_response: dict) -> Tuple[str, str | None, str | None, str | None]:
